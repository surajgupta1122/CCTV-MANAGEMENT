...
function Sidebar() {
  ...
  return (
    <aside
      className={`fixed h-screen bg-white border-r-2 border-gray-300 flex flex-col transition-all duration-300 ease-in-out 
        ${isOpen ? "w-[22%]" : "w-[9%]"}
      `}
    >
    ...
    <div className="flex items-center space-x-2">
      ...
      {isOpen && (<h1 className="text-2xl font-bold text-[#012471]">
        CCTV Manager
        </h1>
      )}
    </div>
    ...
    <li key={item.name}>      
      ...
      <NavLink to={item.path}
        ...
        ${isActive ? "bg-blue-100 font-bold text-blue-900" : "text-gray-500 font-medium " }
        `}>
        ...
        <img src={`src/assets/icons/${item.icon}`} alt={item.name}
        className="w-9 h-9"
        />
